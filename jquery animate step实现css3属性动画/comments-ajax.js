...
addComment = {
	/* 
	commId:当前回复的评论id标识；
	parentId：当前回复的评论id（只包含数字标识部分）；
	respondId：回复框元素id； 
	postId:评论所属文章id；num:可选
	*/
	moveForm : function(commId, parentId, respondId, postId, num) {
		...
		t.I("comment"+parentId).appendChild(respond);
		...
		cancel.onclick = function() {
			...
			$(".post_meta",$("#commentpost")).after($(respond));
			$("html,body").animate( { scrollTop: $('#respond').offset().top - 180 }, 400);
			this.onclick = null;
			return false;
		};
